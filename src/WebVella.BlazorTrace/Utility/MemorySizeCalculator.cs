...
public class MemorySizeCalculator
{
    ...
    public static long CalculateComponentMemorySize<T>(T component, int maxDepth = 2)
    {
        if (component == null)
            throw new ArgumentNullException(nameof(component));
        
        return CalculateSize(component, maxDepth);
    }

    private static long CalculateSize(object obj, int currentDepth, HashSet<object> visited = null)
    {
        ...
        if (type.IsArray)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                if (currentDepth > 0)
                    size += CalculateSize(element, currentDepth - 1, visited);
                else
        ...
        else if (type.IsValueType)
        {
            ...
            else
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    if (currentDepth > 0)
                        size += CalculateSize(fieldValue, currentDepth - 1, visited);
                    else
