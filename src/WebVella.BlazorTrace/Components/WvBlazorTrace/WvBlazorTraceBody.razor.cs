...
public partial class WvBlazorTraceBody : WvBlazorTraceComponentBase, IAsyncDisposable
{
	...
	private string _helpLink = "https://github.com/WebVella/WebVella.BlazorTrace/wiki/Help:-Main";
	...
	private async Task _saveSnapshot()
	{
		...
		try
		{
			var snapshot = await WvBlazorTraceService.CreateSnapshotAsync(JSRuntimeSrv);
	...
	private async Task _renameSnapshot(WvSnapshotListItem sn)
	{
		...
		try
		{
			await WvBlazorTraceService.RenameSnapshotAsync(JSRuntimeSrv, sn.Id, sn.Name);
